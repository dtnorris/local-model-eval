...
[[ -n "$QUEUE_ARG" ]] || { echo "Usage: ./run_production_backlog_gmpb.sh production_backlog/QUEUE"; exit 2; }
...
if [[ -f "$PAUSE_FILE" ]]; then
  ...
  echo "  ./resume_production_backlog_gmpb.sh $QUEUE_ARG"
  exit 0
fi

"$VERIFY" "$QUEUE_ARG" || { echo "ERROR: GMPB backlog preflight failed. No inference launched."; exit 1; }
...
echo
echo "INTERRUPTIBLE GM PREPARATION BURDEN PRODUCTION"
...
while IFS= read -r f; do
  ...
  if [[ -f "$PAUSE_FILE" ]]; then
    ...
    echo "Resume with: ./resume_production_backlog_gmpb.sh $QUEUE_ARG"
    exit 0
  fi
  ...
  echo
  echo "================================================================"
  echo "[$n/$total] GM PREPARATION BURDEN PRODUCTION: $f"
  echo "================================================================"
...
echo
echo "================================================================"
echo "GM PREPARATION BURDEN PRODUCTION QUEUE FINISHED"
echo "================================================================"
echo "Queue: $QUEUE_ARG"
