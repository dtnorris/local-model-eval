...
[[ -n "$QUEUE_ARG" ]] || {
  echo "Usage: ./verify_production_backlog_017.sh production_backlog/production-backlog-017"
  exit 2
}
...
EXPECTED_IDS = (381..403).map { |n| format("ADV-%04d", n) }.freeze
SOURCE_BOUNDARY_CLAMP_IDS = %w[
  ADV-0397 ADV-0400 ADV-0401
].freeze
EXPECTED_DIMENSIONS = [
...
abort "wrong Batch 17 contract type" unless snapshot.fetch("contract_type") == "adventure_ingest_v1"
abort "wrong Batch 17 queue name" unless snapshot.fetch("queue") == "production-backlog-017"
...
abort "AMC missing required Batch 17 columns: #{missing.join(', ')}" unless missing.empty?
...
workbook_rows.each do |row|
  ...
  abort "#{id} Batch 17 score field is no longer blank: #{populated.join(', ')}" unless populated.empty?
...
abort "AMC Batch 17 target IDs/order changed" unless selected_rows == EXPECTED_IDS
...
rows.each_with_index do |row, index|
  ...
  abort "wrong queue in #{relative_manifest}" unless contract["queue"] == "production-backlog-017"
...
puts "Frozen Batch 17 queue semantics: PASS"
puts "  Adventures: 23"
...
[[ "$ACTIVE_SCORER" == "$FROZEN_SCORER" ]] || {
  echo "ERROR: active scorer HEAD is $ACTIVE_SCORER; Batch 17 froze $FROZEN_SCORER"
  exit 1
}

git -C "$SCORER_REPO" diff --quiet -- lib/af_scoring config/default.yml config/source_mappings.yml || {
  echo "ERROR: unstaged scorer/runtime changes touch Batch 17 protected paths"
  exit 1
}
git -C "$SCORER_REPO" diff --cached --quiet -- lib/af_scoring config/default.yml config/source_mappings.yml || {
  echo "ERROR: staged scorer/runtime changes touch Batch 17 protected paths"
  exit 1
}
echo "Scorer checkout: PASS ($ACTIVE_SCORER)"

REPRESENTATIVE_LIST="$(mktemp "${TMPDIR:-/tmp}/af-b017-representative.XXXXXX")" || exit 1
MODEL_LIST="$(mktemp "${TMPDIR:-/tmp}/af-b017-models.XXXXXX")" || {
