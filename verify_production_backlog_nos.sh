...
QUEUE_ARG="${1:-}"
QUALIFIED="$REPO/production_backlog/qualified_number_of_sessions.yml"
SCORER_REPO="${AF_SCORER_REPO:-}"

[[ -n "$QUEUE_ARG" ]] || { echo "Usage: ./verify_production_backlog_nos.sh production_backlog/QUEUE"; exit 2; }
...
abort "wrong B008 contract type" unless qualified.fetch("contract_type") == "nos_local_qualified_v1"
...
abort "wrong dimension" unless snapshot.fetch("dimension") == qualified.fetch("dimension") && qualified.fetch("dimension") == "# of Sessions"
...
abort "wrong qualification boundary" unless Integer(snapshot.fetch("max_page_count_exclusive")) == 100
...
abort "duplicate adventure IDs in B008" unless selected.map { |a| a.fetch("id") }.uniq.length == selected.length
...
rows.each do |row|
  ...
  abort "wrong page boundary in #{path}" unless Integer(contract["max_page_count_exclusive"]) == 100
...
puts "Frozen B008 queue semantics: PASS"
...
puts "  Qualified page domain: 1-99"
RUBY
...
while IFS= read -r protected; do
  PROTECTED_SCORER_FILES+=("$protected")
done < <(ruby - "$QUALIFIED" <<'RUBY'
require "yaml"; YAML.safe_load_file(ARGV.fetch(0)).fetch("protected_scorer_files").each { |p| puts p }
RUBY
)

echo
echo "Checking qualified NoS scorer lineage..."
...
git -C "$SCORER_REPO" merge-base --is-ancestor "$QUALIFIED_BASE" HEAD || { echo "ERROR: scorer HEAD is not a descendant of qualified baseline $QUALIFIED_BASE"; exit 1; }
for protected in "${PROTECTED_SCORER_FILES[@]}"; do
  git -C "$SCORER_REPO" cat-file -e "$QUALIFIED_BASE:$protected" 2>/dev/null || { echo "ERROR: qualified baseline missing $protected"; exit 1; }
  [[ -f "$SCORER_REPO/$protected" ]] || { echo "ERROR: scorer HEAD missing $protected"; exit 1; }
done
git -C "$SCORER_REPO" diff --quiet "$QUALIFIED_BASE"..HEAD -- "${PROTECTED_SCORER_FILES[@]}" || { echo "ERROR: qualified NoS implementation changed after $QUALIFIED_BASE"; git -C "$SCORER_REPO" diff --name-only "$QUALIFIED_BASE"..HEAD -- "${PROTECTED_SCORER_FILES[@]}"; exit 1; }
git -C "$SCORER_REPO" diff --quiet -- "${PROTECTED_SCORER_FILES[@]}" || { echo "ERROR: unstaged changes touch qualified NoS implementation"; exit 1; }
git -C "$SCORER_REPO" diff --cached --quiet -- "${PROTECTED_SCORER_FILES[@]}" || { echo "ERROR: staged changes touch qualified NoS implementation"; exit 1; }
echo "NoS scorer lineage: PASS"
echo "  baseline: $QUALIFIED_BASE"
echo "  scorer HEAD: $(git -C "$SCORER_REPO" rev-parse HEAD)"

CURRENT_CATALOG=$(ruby - "$SCORER_REPO" <<'RUBY'
repo = ARGV.fetch(0); require File.join(repo, "lib", "af_scoring", "errors"); require File.join(repo, "lib", "af_scoring", "config"); print AFScoring::Config.new(project_root: repo).catalog_path
RUBY
)
EXPECTED_CATALOG_FILE=$(ruby - "$QUALIFIED" <<'RUBY'
require "yaml"; print YAML.safe_load_file(ARGV.fetch(0)).fetch("catalog").fetch("filename")
RUBY
)
EXPECTED_CATALOG_SHA=$(ruby - "$QUALIFIED" <<'RUBY'
require "yaml"; print YAML.safe_load_file(ARGV.fetch(0)).fetch("catalog").fetch("sha256")
RUBY
)
...
echo
echo "Recomputing exhaustive AMC population..."
ruby - "$QUALIFIED" "$SNAPSHOT" "$INDEX" "$SCORER_REPO" "$CURRENT_CATALOG" <<'RUBY' || exit 1
require "csv"; require "yaml"
qualified_path, snapshot_path, index_path, scorer_repo, catalog_path = ARGV
q = YAML.safe_load_file(qualified_path); snapshot = YAML.safe_load_file(snapshot_path, aliases: true)
...
abort "B008 is not exhaustive or AMC order changed" unless selected == eligible
...
"$SCORER_REPO/bin/af-score" --model "$EXPECTED_MODEL" --dimension "# of Sessions" --preflight --render-preflight "$TMP_PROFILE_DIR/nos.json" "$FIRST_ADV" >/dev/null || { echo "ERROR: NoS zero-inference preflight failed"; exit 1; }
ruby - "$TMP_PROFILE_DIR/nos.json" <<'RUBY' || exit 1
require "json"
...
abort "dedicated blind NoS system prompt absent" unless instructions.include?("independent blind AdventureFinder # of Sessions assessment")
abort "blind NoS task absent" unless input.include?("# Blind Assessment Task")
abort "frozen AFAO 1.6 NoS rubric heading absent" unless input.include?("# Frozen AFAO 1.6 # of Sessions Rubric")
abort "generic production rules contaminated NoS prompt" if input.include?("# AdventureFinder Production Assessment Rules")
abort "legacy compact-phase guardrail contaminated NoS prompt" if input.include?("Compact-Phase Aggregation Guardrail")
abort "calibration benchmarks contaminated NoS prompt" if input.include?("Calibration Benchmarks")
puts "Qualified NoS prompt activation: PASS"
RUBY
...
while IFS= read -r f; do
  [[ -z "$f" ]] && continue
  if ! bin/lme plan "$f" >/dev/null; then echo "PLAN FAILED: $f"; failed=$((failed + 1)); fi
done < "$ORDER"
...
echo
echo "Checking local worker and authorized Qwen 27B model..."
FIRST_MANIFEST=$(head -n 1 "$ORDER")
bin/lme worker-check "$FIRST_MANIFEST" || exit 1

echo
echo "PREFLIGHT PASSED — exhaustive qualified NoS production queue is ready."
