...
[[ -n "$QUEUE_ARG" ]] || {
  echo "Usage: ./verify_production_backlog_019.sh production_backlog/production-backlog-019"
  exit 2
}
...
CATALOG_FILENAME = "5e_Adventure_Master_Catalog_4.9.xlsx"
CATALOG_SHA256 = "57fdcbc021af25e72a482e1b0183e5345df29cbfda7d91119c19ace371a1388e"
EXPECTED_IDS = (429..464).to_a.map { |n| format("ADV-%04d", n) }.freeze
EXPECTED_LEVEL_IDS = EXPECTED_IDS
SOURCE_BOUNDARY_CLAMP_IDS = %w[
  ADV-0447
  ADV-0460
].freeze
SOURCE_BOUNDARY_CLAMP_MAX_GAPS = {
  "ADV-0447" => 2
}.freeze
EXPECTED_DIMENSIONS = [
...
abort "wrong Batch 19 contract type" unless snapshot.fetch("contract_type") == "adventure_ingest_v1"
abort "wrong Batch 19 queue name" unless snapshot.fetch("queue") == "production-backlog-019"
...
abort "wrong expected adventure count" unless Integer(snapshot.fetch("expected_adventure_count")) == 36
...
abort "source-boundary clamp max gaps changed" unless
  snapshot.fetch("source_boundary_clamp_max_gaps") == SOURCE_BOUNDARY_CLAMP_MAX_GAPS
...
runtime_files.each do |key, info|
  ...
  abort "runtime #{key} no longer points to AMC 4.9" unless data.dig("files", "catalog") == CATALOG_FILENAME
  ...
  abort "runtime #{key} source-boundary clamp max gaps changed" unless
    data.dig("source", "inward_boundary_clamp_max_gap_by_adventure") == SOURCE_BOUNDARY_CLAMP_MAX_GAPS
...
abort "AMC missing required Batch 19 columns: #{missing.join(', ')}" unless missing.empty?
...
workbook_rows.each do |row|
  ...
  abort "#{id} Batch 19 score field is no longer blank: #{populated.join(', ')}" unless populated.empty?
...
abort "AMC Batch 19 target IDs/order changed" unless selected_rows.map(&:first) == EXPECTED_IDS
actual_level_ids = selected_rows.select { |_id, needs_levels| needs_levels }.map(&:first)
abort "AMC Batch 19 conditional Levels IDs changed" unless actual_level_ids == EXPECTED_LEVEL_IDS
preserved_level_ids = selected_rows.reject { |_id, needs_levels| needs_levels }.map(&:first)
abort "AMC Batch 19 preserved Levels count changed" unless preserved_level_ids.empty?
...
abort "unexpected Batch 19 dimensions: #{unexpected_dimensions.join(', ')}" unless unexpected_dimensions.empty?
...
rows.each_with_index do |row, index|
  ...
  abort "wrong queue in #{relative_manifest}" unless contract["queue"] == "production-backlog-019"
...
puts "Frozen Batch 19 queue semantics: PASS"
puts "  Adventures: 36"
puts "  Calls: #{EXPECTED_CALLS} (396 ordinary + 36 conditional Levels)"
puts "  Order: adventure-major / 11 ordinary dimensions plus conditional Levels"
puts "  Catalog: #{CATALOG_FILENAME} @ #{CATALOG_SHA256}"
puts "  Levels: #{EXPECTED_IDS.length - EXPECTED_LEVEL_IDS.length} preserved pairs / #{EXPECTED_LEVEL_IDS.length} qualified paired-output calls"
...
[[ "$ACTIVE_SCORER" == "$FROZEN_SCORER" ]] || {
  echo "ERROR: active scorer HEAD is $ACTIVE_SCORER; Batch 19 froze $FROZEN_SCORER"
  exit 1
}

git -C "$SCORER_REPO" diff --quiet -- lib/af_scoring config/default.yml config/source_mappings.yml || {
  echo "ERROR: unstaged scorer/runtime changes touch Batch 19 protected paths"
  exit 1
}
git -C "$SCORER_REPO" diff --cached --quiet -- lib/af_scoring config/default.yml config/source_mappings.yml || {
  echo "ERROR: staged scorer/runtime changes touch Batch 19 protected paths"
  exit 1
}
echo "Scorer checkout: PASS ($ACTIVE_SCORER)"

REPRESENTATIVE_LIST="$(mktemp "${TMPDIR:-/tmp}/af-b019-representative.XXXXXX")" || exit 1
MODEL_LIST="$(mktemp "${TMPDIR:-/tmp}/af-b019-models.XXXXXX")" || {
...
echo
echo "BATCH 19 PREFLIGHT: PASS"
echo "  36 adventures"
echo "  11 ordinary scored fields per adventure"
echo "  36 conditional Levels calls (paired Level Start + Level End)"
echo "  432 frozen local inference calls"
